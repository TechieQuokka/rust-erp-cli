...
impl ConfigHandler {
    pub async fn handle(cmd: &ConfigCommands, config: &AppConfig) -> ErpResult<()> {
        ...
        match cmd {
            ...
            ConfigCommands::List { filter } => Self::handle_list(filter, &service).await,
    ...
    async fn handle_list(filter: &Option<String>, service: &ConfigService) -> ErpResult<()> {
        let term = Term::stdout();
        ...
        if configs.is_empty() {
            let _ = term.write_line(&style("No configurations found.").yellow().to_string());
            return Ok(());
        }
